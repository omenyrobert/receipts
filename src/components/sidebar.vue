<template>
  <div class="p-5 h-full w-full">
    ...
    <div v-if="isOpen" class="border border-gray-200 rounded-md absolute w-1/2 shadow-2xl ml-[15vw] bg-white">
      <form @submit.prevent="postreceipt">
        ...
        <div class="flex p-3">
          <div class="w-1/2">
            ...
            <input type="text" v-model="product" placeholder="Enter Product"
              class="border border-gray-300 w-3/4 rounded-md p-2 mt-2" required />
            <br /><br />
            <label>Contacts</label><br />
            <input type="text" v-model="contacts" placeholder="Enter contacts"
              class="border border-gray-300 w-3/4 rounded-md p-2 mt-2" required />
          </div>
          <div class="w-1/2">
            <label>Company Name</label><br />
            <input type="text" v-model="company" placeholder="Enter Name"
              class="border border-gray-300 w-3/4 rounded-md p-2 mt-2" required />
            <br /><br />
            ...
            <input type="text" v-model="address" placeholder="Enter Address"
              class="border border-gray-300 w-3/4 rounded-md p-2 mt-2" required />
            <br /><br /><br />
            <button type="submit" class="text-white p-2 bg-blue-700 rounded-md w-3/4">
              Add Receipt
            </button>
          </div>
        </div>
        ...
        <div v-for="(item, index) in invoiceItems" :key="index">
          <div class="flex">
            <div class="p-1">
              <input v-model="item.item" placeholder="Enter Item"
                class="border border-gray-300 w-full rounded-md p-2 mt-2" required />
            </div>
            <div class="p-1">
              <input v-model="item.qty" placeholder="qty" type="number"
                class="border border-gray-300 w-full rounded-md p-2 mt-2" required />
            </div>
            <div class="p-1">
              <input v-model="item.unitCost" type="number" placeholder="Unit cost"
                class="border border-gray-300 w-full rounded-md p-2 mt-2" required />
            </div>
            ...
            <br />
            <div>
              <button type="button" class="text-red-700 text-xs mt-5" @click="removeItem(index)" v-show="index != 0">
                Remove
              </button>
            </div>
          </div>
    ...
    <br />
    <RouterLink to="/" class="m-1 text-blue-700 cursor-pointer rounded-md p-2 flex text-center">
      <FolderIcon class="w-3 mt-1" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Invoices
    </RouterLink>

    <RouterLink to="/receipts" class="m-1 text-blue-700 cursor-pointer rounded-md p-2 flex text-center">
